...
class VistaRitiriAziendaliNegativi(QWidget):
    def __init__(self, gestoreRitiro):
        super().__init__()
        self.gestoreRitiro = gestoreRitiro
        self.resize(600, 250)
        self.setWindowTitle("Ritiri aziendali non andati a buon fine")
        self.label = QLabel("Ritiri aziendali rimandati:")
        self.info_ritiro = QPushButton("Visualizza info")  # Inizializzazione di info_ritiro
    ...
    def initUI(self):
        vlayout = QVBoxLayout()
        hlayout = QHBoxLayout()
        self.listaRitiriNegativi = QListWidget()  # Usato il nome giusto per la lista
        self.label.setStyleSheet("font-size: 15px; font-family: Arial; font-weight:bold;")
        i = 0
        for ritiroNegativo in self.gestoreRitiro.listaColliNegativi:
            self.listaRitiriNegativi.insertItem(i, f"Collo n. {ritiroNegativo.datiRitiro.codiceRitiro}")
            i += 1
    ...
    def show_selected_info(self, gestoreRitiro):
        try:
            ...
            if ritiro_selezionato is not None:
                # Selezionato un elemento dalla lista
